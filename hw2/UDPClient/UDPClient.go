...
func main() {
	...
	for {
		...
		if num == "1\n" {
			command_1(num, pconn, serverName, serverPort)
		} else if num == "2\n" {
			command_2(num, pconn, serverName, serverPort)
		} else if num == "3\n" {
			command_3(num, pconn, serverName, serverPort)
		} else if num == "4\n" {
			command_4(num, pconn, serverName, serverPort)
		} else if num == "5\n" {
			pconn.Close()
			f.Print("Bye bye~")
			os.Exit(0)
...
func command_1(num string, conn net.PacketConn, serverName string, serverPort string) {
	...
	conn.WriteTo([]byte(num+input), serverAddr)
...
func command_2(num string, conn net.PacketConn, serverName string, serverPort string) {
	startTime := time.Now()
	serverAddr, _ := net.ResolveUDPAddr("udp", serverName+":"+serverPort)
	conn.WriteTo([]byte(num), serverAddr)
...
func command_3(num string, conn net.PacketConn, serverName string, serverPort string) {
	startTime := time.Now()
	serverAddr, _ := net.ResolveUDPAddr("udp", serverName+":"+serverPort)
	conn.WriteTo([]byte(num), serverAddr)
...
func command_4(num string, conn net.PacketConn, serverName string, serverPort string) {
	startTime := time.Now()
	serverAddr, _ := net.ResolveUDPAddr("udp", serverName+":"+serverPort)
	conn.WriteTo([]byte(num), serverAddr)
